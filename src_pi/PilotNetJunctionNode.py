...
import zmq
import time
from raspberrypi_car.src_pi.utils.message_utils import create_json_message, parse_json_message, parse_jpg_image_message
from raspberrypi_car.src_pi.Node import Node
import tensorflow as tf
from PilotNetJunctionRaspiCarBase import BasePilotNetRaspiCar, stack_branches_func, mask_outputs, resize_and_crop_image_tf, preprocess_image
import numpy as np

def preprocess_command(road_option, pilotnet_helper):
    """
    Turns the road_option enum into one-hot for the model.
    """
    cmd_idx = pilotnet_helper.control_command_to_index.get(
        road_option,
        pilotnet_helper.control_command_to_index['STRAIGHT']
    )
    cmd_one_hot = tf.keras.utils.to_categorical(
        cmd_idx,
        num_classes=pilotnet_helper.total_control_commands
    )
...
class PilotNetJunctionNode(Node):
    ...
    def __init__(self, log_level=logging.INFO,
                 ...
                 gamepad_sub_url="tcp://raspberrypi.local:5541",
        ...
        self.PilotNet = BasePilotNetRaspiCar(params={
            "data_dirs": {"": 0},
            "save_dir": "",
            "target_width": 200,
            "target_height": 66,
        ...
        # Path to your model
        self.model = self._load_model("/source/training/03-03-2025_14-41/checkpoints/cp-0021.keras")
    ...
    def _process_camera_message(self):
        ...
        road_option = self.current_gamepad_msg.get("control_command", "STRAIGHT")
        command_tensor = preprocess_command(road_option, self.PilotNet)
        ...
        self.log(f"Steering Prediction: {pilotnet_steering_angle} | Prob: {np.max(class_probs):.3f} | Command Input: {road_option}", logging.DEBUG)
...
if __name__ == "__main__":
    subscriber_node = PilotNetJunctionNode(log_level=logging.DEBUG, use_weighted_average=True)
    try:
